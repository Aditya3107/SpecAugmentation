...
# X train SpecFeature extraction 
X_train_Specfeatures = pd.DataFrame(columns = ['features'])
counter = 0
for index,path in enumerate(X_train.path):
    X, sample_rate = librosa.load(path, res_type='kaiser_fast', duration = 2.5, sr = 22050*2, offset = 0.5)
    sample_rate = np.array(sample_rate)
    #taking mean of MFCC 
    mfccs = librosa.feature.melspectrogram(y = X, sr = sample_rate, n_mels=256,hop_length=128,fmax=8000)
    warped_masked_spectrogram = np.mean((spec_augment_pytorch.spec_augment(mel_spectrogram=mfccs)),axis = 0)
    X_train_Specfeatures.loc[counter] = [warped_masked_spectrogram]
    counter = counter+1
X_train_Specfeatures = (pd.DataFrame(X_train_Specfeatures['features']. values.tolist())).fillna(0)

frames = [X_train_features, X_train_Specfeatures]

X_train_features_all = (pd.concat(frames, axis = 0)).fillna(0)

#result.to_csv('CSVfiles/results.csv', index = False, header=True)
#print(result.info)
...
mean = np.mean(X_train_features_all, axis=0)
std = np.std(X_train_features_all, axis=0)

X_train_features = ((X_train_features_all - mean)/std).fillna(0)
...
model_name = 'Emotion_recognition_with_specAugment.h5'
